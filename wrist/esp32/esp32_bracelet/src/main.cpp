...
unsigned long ground_zeros_charge = 0; // for time measurement and send information about battery charge
unsigned long ground_zeros_imu = 0; // for time measurement and sending data from imu
...
uint8_t broadcastAddress_pc[] = {0x54, 0x43, 0xB2, 0xE5, 0xC6, 0x3C};
...
void inform_about_charge(void) {
    ...
    if (ground_zeros_charge > current_ticks) {
        current_ticks += (0xFFFFFFFF - ground_zeros_charge);
        ground_zeros_charge = 0;
    }

    if (current_ticks - ground_zeros_charge >= CHARGE_INFO_TIME) {
    ...
    if (uart_flag) {
        ...
        if (!break_flag) {
            if (!is_gesture_defined) {
                ...
                send_data_to_device(&switching_struct);
                digitalWrite(RX_1_LED, 1);
            }
...
void send_data_to_device(imu_data_s *ptr_struct) {
    switch (bracelet_mode) {
        case 0:
            esp_now_send(broadcastAddress_pc, (uint8_t *) ptr_struct, sizeof(*ptr_struct));
            break;
        case 1:
            esp_now_send(broadcastAddress_car, (uint8_t *) ptr_struct, sizeof(*ptr_struct));
            break;
        case 2:
            esp_now_send(broadcastAddress_tank, (uint8_t *) ptr_struct, sizeof(*ptr_struct));
            break;            
    }
}
...
void mpu_init(void) {
    ...
    if (status < 0) {
        ...
        IMU.setAccelCalY(ACC_Y_BIAS, ACC_Y_SCALE);
...
void mpu_data_processing(void) {
    unsigned long current_ticks = millis();
    if (ground_zeros_imu > current_ticks) {
        current_ticks += (0xFFFFFFFF - ground_zeros_imu);
        ground_zeros_imu = 0;
    }

    if (current_ticks - ground_zeros_imu >= IMU_INFO_TIME) {
        IMU.readSensor();

        imu_data.a_x = constrain(IMU.getAccelX_mss() * 102, -1000, 1000);
        imu_data.a_y = constrain((IMU.getAccelY_mss() /*+ 4*/) * 102, -1000, 1000);
        // Serial.print(imu_data.a_x); Serial.print('\t'); Serial.println(imu_data.a_y);
#ifndef UAV_CONTROL
        imu_data.a_z = constrain(IMU.getAccelZ_mss() * 102, -1000, 1000);
        imu_data.w_x = IMU.getGyroX_rads() * 100; // TODO: 100 - is temporary variable
        imu_data.w_y = IMU.getGyroY_rads() * 100; // TODO: 100 - is temporary variable
        imu_data.w_y = IMU.getGyroZ_rads() * 100; // TODO: 100 - is temporary variable
#endif

        send_data_to_device(&imu_data);
        ground_zeros_imu = millis();
    }
}
